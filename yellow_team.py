...
class Player:
    def __init__(self, name, role, color, ball, position_x, position_y):
        self.name = name
        self.role = role
        self.color = color
        self.ball = ball
...
class ClientHandle:
    ...
    def catch_ball_first(self, all_data):
        blue_one = all_data["yellow_team"]["yellow_one"]
        ball = all_data["ball"]
    ...
    def get_closest_player_to_enemy_w_ball(self, all_data):
        ...
        closest_player_name = "yellow_one"
        not_goalkeeper = False
        while not_goalkeeper:
            min_distance = distance.euclidean([player_wb["position_x"], player_wb["position_y"]],
                                              [all_data["yellow_team"][closest_player_name]["position_x"],
                                               all_data["yellow_team"][closest_player_name]["position_y"]])

            for player_name in all_data["yellow_team"]:
                player = all_data["yellow_team"][player_name]
                tmp_dist = distance.euclidean([player_wb["position_x"], player_wb["position_y"]],
                                              [player["position_x"], player["position_y"]])
                if tmp_dist < min_distance:
                    closest_player_name = player_name
            if all_data["yellow_team"][closest_player_name]["role"] == "simple":
                not_goalkeeper = True
    ...
    def follow_ball(self, closest_player, all_data):
        ...
        if all_data["yellow_team"][closest_player]["role"] == "simple":
            all_data["yellow_team"][closest_player]["position_x"] += (player_wb["position_x"] - all_data["yellow_team"][closest_player]["position_x"]) / speed
            all_data["yellow_team"][closest_player]["position_y"] += (player_wb["position_y"] - all_data["yellow_team"][closest_player]["position_y"]) / speed

    def move_players_to_enemy_gate(self, all_data):
        speed = (random.randint(1, 20)/10)
        enemy_gate = all_data["blue_gate"]
        for player in all_data["yellow_team"]:
            if all_data["yellow_team"][player]["role"] == "simple":
                dist_x = enemy_gate["position_x"]- all_data["yellow_team"][player]["position_x"]
                dist_y = enemy_gate["position_y"]- all_data["yellow_team"][player]["position_y"]
                a = int(("1" + (len(str(abs(dist_x)))-1)*"0"))+10
                b = int(("1" + (len(str(abs(dist_x)))-1)*"0"))+10
                all_data["yellow_team"][player]["position_x"] += (dist_x / a)*speed
                all_data["yellow_team"][player]["position_x"] += (dist_y / b)*speed
    ...
    def run(self):
        while True:
            ...
            if all_data["player_with_ball"]["name"] == None:
                ...
                if all_data["player_with_ball"]["color"] == "blue":
